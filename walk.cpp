...
inline double get_edge(const std::vector<double>& matrix, size_t n, size_t from, size_t to) {
    return matrix[from * n + to];
}

double compute_tour_cost(const std::vector<int>& tour, const std::vector<double>& matrix, size_t n, bool closed) {
    double cost = 0.0;
    for (size_t i = 0; i < n - 1; ++i) {
        cost += get_edge(matrix, n, tour[i], tour[i + 1]);
    }
    if (closed) cost += get_edge(matrix, n, tour.back(), tour.front());
    return cost;
}

void optimize_tsp_2opt(std::vector<int>& tour, const std::vector<double>& matrix, size_t n, bool closed, double& best_cost) {
    bool improved = true;
    while (improved) {
        improved = false;
...
void solve_tsp_aco(const std::vector<double>& matrix, size_t n, bool closed, int generations, int ants, double evaporation, double beta, std::vector<int>& global_best_tour, double& global_best_cost) {
    ...
    #pragma omp parallel
    {
        ...
        for (int start_node = 0; start_node < (int)n; ++start_node) {
            ...
            for (size_t step = 1; step < n; ++step) {
                ...
                for (size_t cand = 0; cand < n; ++cand) {
                    if (!visited[cand]) {
                        ...
                        if (d < min_dist) { min_dist = d; next_node = cand; }
                    }
                }
            ...
            if (cost < local_best_cost) { local_best_cost = cost; local_best_tour = tour; }
        }
        #pragma omp critical
        {
    ...
    for (size_t i = 0; i < n * n; ++i) heuristic[i] = std::pow(1.0 / (matrix[i] + 1e-6), beta);
    std::vector<double> weights(n * n);

    // Phase 2: ACO Iterations
    for (int gen = 0; gen < generations; ++gen) {
        for (size_t i = 0; i < n * n; ++i) weights[i] = pheromone[i] * heuristic[i];
            ...
            for (int a = 0; a < ants; ++a) {
                ...
                for (size_t step = 1; step < n; ++step) {
                    ...
                    for (size_t i = 0; i < n; ++i) {
                        if (!visited[i]) {
                            cumulative += probs[i];
                            if (cumulative >= threshold) { next_node = i; break; }
                        }
                    }
                    if (next_node == -1) { 
                        for(size_t i=0; i<n; ++i) if(!visited[i]) { next_node = i; break; }
                    }
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, short_opts, long_opts, nullptr)) != -1) {
        switch (opt) {
            ...
            case 'p': is_closed_loop = true; break;
            case 'r': restarts = std::stoi(optarg); break;
            case 'a': ants = std::stoi(optarg); break;
            case 'e': evaporation = std::stod(optarg); break;
            case 'b': beta = std::stod(optarg); break;
            default:
                std::cerr << "Usage: " << argv[0] << " -i <in.json> -o <out.json> [-m duration|distance] [--closed] [-r generations] [--ants int] [--evap float] [--beta float]\n";
                return 1;
        }
    }
    ...
    std::cerr << "INFO: OpenMP Multi-threading Active (" << omp_get_max_threads() << " threads).\n";

    std::vector<int> optimal_indices;
    double final_cost;
    solve_tsp_aco(flat_matrix, n, is_closed_loop, restarts, ants, evaporation, beta, optimal_indices, final_cost);

    std::vector<std::string> optimal_names;
    for (int idx : optimal_indices) optimal_names.push_back(names[idx]);
